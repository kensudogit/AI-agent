...
const sections = [
  {
    ...
    items: [
      'PDF または Excel（.xlsx / .xls）をチャットからアップロードできます。入力欄左のクリップ（📎）ボタンでファイルを選ぶと、サーバーがテキストを抽出し、AI が要約・構成評価・リスク・改善提案などを返します。評価の観点を入力欄に書いてからアップロードすると、その指示が反映されます（15MB 以下）。',
  ...
  },
  {
    ...
    items: [
      ...
      '「シナリオを選ぶ」のリモコン盤を開き、次の3つを選択します。（1）シナリオ：B2B SaaS・価格納期・初回訪問・人材紹介・広告契約・外注・エンタープライズライセンス・更新契約・業務提携MOU・賃貸オフィス・コンサル報酬・保守SLA・基幹遅延交渉・高齢IT派遣などから選びます。（2）難易度：易・標準・難（相手AIの態度が協力的〜厳しめに変わります）。（3）あなたの役割：営業側（AIが顧客）か顧客側（AIが営業）。',
      '「商談を開始」を押すとセッションが始まります。AIが商談の最初の一言を表示するので、その内容に合わせて返答を入力します。画面上部に経過時間が表示されます。開始後は「商談準備」で本日のゴール・メモを残せます（リロード後もブラウザに保存）。その下に「論点チェック」が常に表示され、本番に近い形で論点を押さえられます。',
      '返答は複数行のテキスト欄に入力します（Enter で送信、Shift+Enter で改行。最大2000文字）。「会話をコピー」で全文をまとめてコピーでき、各吹き出しの「コピー」で1通だけコピーできます。マイクで音声入力も可能です。AIは選択したシナリオ・役割・難易度に合わせて相手役を演じます。',
      '一通り商談が終わったら「終了してフィードバック」を押します。AIがログを分析し、良かった点・改善点・アドバイス・総合スコア（1〜5）を表示します。フィードバックは厳格・辛辣モード（忖度なし・基準厳しめ）で生成されます。',
      '「履歴」で過去のセッション一覧を表示できます。各セッションをクリックすると、会話ログとフィードバックを振り返れます（DB設定時のみ保存）。',
    ],
  },
  {
    id: 3,
    ...
    items: [
      ...
      'チャット・模擬商談の応答には Anthropic Claude API を使用します。ANTHROPIC_API_KEY が未設定または無効な場合、「ANTHROPIC_API_KEY not configured」や 503 などのエラーになります。管理者は環境変数を確認してください。',
